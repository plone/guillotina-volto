...
class Site(Container):
    async def install(self):
        ...
        new_group = await create_content_in_container(groups, "Group", "managers", check_security=False, **payload)

        users = await self.async_get("users")
        new_user: IUser = await create_content_in_container(users, "User", "admin", check_security=False)
