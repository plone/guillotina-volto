...
async def test_constraints(cms_requester):
    async with cms_requester as requester:
        ...
        # Check constraints
        resp, status = await requester("GET", "/db/guillotina/doc1/@constraints")
        ...
        resp, status = await requester(
            "POST",
            "/db/guillotina/doc1/",
            data=json.dumps({"@type": "File", "title": "File 1", "id": "fil1"}),
        )
        ...
        resp, status = await requester(
            "POST",
            "/db/guillotina/doc1/",
            data=json.dumps({"@type": "Image", "title": "Image 1", "id": "img1"}),
        )
