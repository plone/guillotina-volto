...
async def test_vocabulary(cms_requester):
    resp, status = await cms_requester("GET", "/db/guillotina/@vocabularies/content_types")
    assert status == 200
    assert "Page" in list(map(lambda x: x["title"], resp["items"]))
    assert resp["items_total"] == 9

    resp, status = await cms_requester("GET", "/db/guillotina/@vocabularies/workflow_states")
    assert status == 200
